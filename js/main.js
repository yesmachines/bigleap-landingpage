...
function initSlider({
  ...
  ["mouseleave", "focusout"].forEach((e) =>
    root.addEventListener(e, startAuto),
  );
  const onDown = (x) => {
    dragging = !0;
    startX = x;
...
(function () {
  ...
  document.querySelectorAll("[data-yt-open]").forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      open(
        id,
        btn.querySelector(".youtube-card-title")?.textContent?.trim() ||
          "Video",
  ...
  modal
    .querySelectorAll("[data-yt-close]")
...
(function () {
  ...
  const validators = {
    ...
    company(value) {
      ...
      if (v.length > 100) return "Company name is too long.";
      return "";
    },
    service(value) {
  ...
  const submitViaWeb3Forms = async (data) => {
    ...
    try {
      ...
      throw new Error(
        "Could not connect to email service. Check your internet and try again.",
      );
    }
    if (!result.success) {
      throw new Error(
        result.message ||
          "Could not send your enquiry. Please try again or call us.",
      );
    }
  };
