function initSlider({ rootId, trackId, dotsId, viewportSel, slidesHtml, perView, autoMs = 4500, autoHeight = !1 }) {
    ...
    const layout = () => {
        ...
        const w = slideW();
        slides.forEach((s) => {
            s.style.width = autoHeight ? w + "px" : 100 / pv + "%";
        });
    ...
    viewport.addEventListener("touchstart", (e) => onDown(e.touches[0].clientX), { passive: !0 });
    viewport.addEventListener("touchmove", (e) => onMove(e.touches[0].clientX), { passive: !0 });
...
(function () {
    ...
    const open = (id, title, start = 0) => {
        ...
        videoWrap.querySelector("iframe")?.addEventListener(
            "load",
            () => {
    ...
    document.querySelectorAll("[data-yt-open]").forEach((btn) => {
        btn.addEventListener("click", () => {
            ...
            open(
                id,
                btn.querySelector(".youtube-card-title")?.textContent?.trim() || "Video",
                parseInt(btn.getAttribute("data-yt-start") || "0", 10)
            );
        });
    });
...
(function () {
    ...
    const websiteForEmail = (value) => String(value || "").trim();
    const validators = {
        firstName(value) {
            const v = value.trim();
        ...
        website(value) {
            if (value.trim().length > 2048) return "Website is too long.";
            return "";
        },
        service(value) {
    ...
    const validateField = (name) => {
        ...
        if (!input || !validators[name]) return !0;
    ...
    const submitToBackend = async (data) => {
        await Promise.all([submitToGas(data), submitViaWeb3Forms(data)]);
    };
...
(function () {
    ...
    document.addEventListener("mousemove", onMove, { passive: !0 });
